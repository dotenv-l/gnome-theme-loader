...
class ThemeCard(Gtk.Box):
    ...
    def __init__(self, name, theme_type, path, apply_callback, preview_callback=None, delete_callback=None, is_applied=False, description=None):
        ...
        self.theme_description = description or ""
        self.theme_path = str(path) if path else ""
        self.path = path
        ...
        header.append(thumb)
        # Información del tema (fuente más pequeña)
        info_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)
        name_label = Gtk.Label(label=name)
        name_label.set_css_classes(["theme-name-compact"])
        ...
        if description:
            desc_label = Gtk.Label(label=description[:60] + ("..." if len(description) > 60 else ""))
            desc_label.set_css_classes(["caption", "dim-label", "desc-compact"])
        ...
        # Botón de aplicar
        self.apply_btn = Gtk.Button(label="Aplicar" if not is_applied else "Aplicado")
        self.apply_btn.set_css_classes(["pill"])
        if is_applied:
            self.apply_btn.set_sensitive(False)
            self.apply_btn.add_css_class("applied-btn")
        else:
        ...
        if is_applied:
            ...
            applied_label.set_css_classes(["caption", "theme-applied-label-compact"])
        ...
        if path:
            ...
            path_label.set_css_classes(["caption", "dim-label", "theme-path-compact"])
