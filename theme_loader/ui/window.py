...
class Window(Adw.ApplicationWindow):
    ...
    def _connect_menu_actions(self):
        ...
        # Acción de acerca de (ahora muestra modal)
        about_action = Gio.SimpleAction.new("about", None)
        about_action.connect("activate", lambda action, param: self._show_about_dialog())
    ...
    def _build_ui(self):
        ...
        self.search_entry.set_placeholder_text("Buscar temas...")
        self.search_entry.connect("changed", self._on_search_changed)
        self.search_bar.connect_entry(self.search_entry)
        self.search_bar.set_child(self.search_entry)
        main_box.append(self.search_bar)
        # Conectar cierre de barra de búsqueda de forma compatible con GTK4
        def on_search_mode_notify(search_bar, param):
        ...
        self.search_bar.connect("notify::search-mode-enabled", on_search_mode_notify)
        ...
        # Botón de instalación manual flotante (OCULTO)
    ...
    def _create_header_right_buttons(self, header_bar):
        ...
        # Botón de tienda en línea (mundito)
        store_button = Gtk.Button()
        store_button.set_icon_name("applications-internet-symbolic")
        store_button.set_tooltip_text("Abrir tienda en línea")
        store_button.connect("clicked", lambda *_: self._show_store_window())
        header_bar.pack_end(store_button)
        # Búsqueda
        ...
        self.search_button = search_button  # Guardar referencia para control
        # Refrescar
    ...
    def _filter_themes(self, search_text: str):
        """Filtrar temas por nombre, descripción o ruta (reconstruyendo el grid, compatible GTK4)"""
        if not hasattr(self, 'theme_grids'):
            return
        current_grid = self.theme_grids.get(self.current_theme_type)
        if not current_grid:
            return
        # Recuperar la lista original de temas
        all_cards = getattr(self, '_all_cards', None)
        if all_cards is None:
            # Guardar la lista original de tarjetas
            all_cards = []
        ...
        # Limpiar el grid (recorrido manual)
        to_remove = []
        child = current_grid.get_first_child()
        while child:
        ...
        for card in to_remove:
            current_grid.remove(card)
        # Filtrar y agregar
        search_text = search_text.strip().lower()
        for card in self._all_cards:
            if any(search_text in (getattr(card, attr, '').lower()) for attr in ['theme_name', 'theme_description', 'theme_path']) or not search_text:
                current_grid.append(card)
        # Si el filtro está vacío y faltan tarjetas, restaurar todas
        if not search_text:
            # Asegura que todas estén presentes
            present = set()
            child = current_grid.get_first_child()
            while child:
                present.add(child)
    ...
    def _show_store_window(self):
        """Mostrar demo de la tienda de temas en línea (en desarrollo)"""
        dialog = Adw.MessageDialog(
            transient_for=self,
            heading="Tienda en línea (Demo)",
            body="La tienda de temas en línea estará disponible próximamente.\n\nAquí podrás explorar y descargar temas directamente desde GNOME-Look.org.\n\nFuncionalidad en desarrollo."
        )
        dialog.add_response("close", "Cerrar")
        ...
        # Título
        title = Gtk.Label(label="Tienda en línea (Demo)")
        title.set_css_classes(["heading"])
        title.set_halign(Gtk.Align.CENTER)
        content_area.append(title)

        # Descripción
        desc = Gtk.Label(label="Próximamente podrás explorar y descargar temas directamente desde GNOME-Look.org, sin salir de la app.\n\nMientras tanto, puedes visitar la tienda online desde el siguiente enlace:")
        desc.set_wrap(True)
        ...
        # Enlace directo
        link = Gtk.LinkButton(uri="https://www.gnome-look.org/", label="Ir a GNOME-Look.org")
        link.set_halign(Gtk.Align.CENTER)
        content_area.append(link)

        # Mensaje de desarrollo
        dev_msg = Gtk.Label(label="Funcionalidad en desarrollo. ¡Gracias por tu paciencia!")
        dev_msg.set_css_classes(["caption"])
        dev_msg.set_halign(Gtk.Align.CENTER)
        content_area.append(dev_msg)
    ...
    def _show_about_dialog(self):
        """Mostrar diálogo modal de Acerca de"""
        dialog = Adw.MessageDialog(
            transient_for=self,
            heading="Acerca de GNOME Theme Loader",
            body="Una herramienta moderna para gestionar y aplicar temas en GNOME.\n\nDesarrollado por dotenv-l.\n\nRepositorio: https://github.com/dotenv-l/gnome-theme-loader"
        )
        dialog.add_response("close", "Cerrar")
        dialog.set_response_appearance("close", Adw.ResponseAppearance.SUGGESTED)
        ...
        # Nombre y versión
        title = Gtk.Label(label="GNOME Theme Loader v1.0")
        title.set_css_classes(["heading"])
        title.set_halign(Gtk.Align.CENTER)
        content_area.append(title)

        # Descripción
        desc = Gtk.Label(label="Una herramienta moderna para gestionar y aplicar temas en GNOME.")
        desc.set_wrap(True)
        desc.set_justify(Gtk.Justification.CENTER)
        desc.set_halign(Gtk.Align.CENTER)
        content_area.append(desc)
        ...
        # Equipo
        team = Gtk.Label(label="Desarrollado por dotenv-l")
        team.set_css_classes(["caption"])
        team.set_halign(Gtk.Align.CENTER)
        content_area.append(team)

        # Enlace al repositorio (actualizado)
        repo_link = Gtk.LinkButton(uri="https://github.com/dotenv-l/gnome-theme-loader", label="Repositorio en GitHub")
        repo_link.set_halign(Gtk.Align.CENTER)
        content_area.append(repo_link)
